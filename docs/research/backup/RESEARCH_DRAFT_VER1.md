...
しかし、このようなモノリシックなVLAモデルには課題が存在する。第一に、高品質なロボット操作データの収集には多大なコストがかかり、データの規模・多様性が限定される[3, 29, 30]。第二に、VLAモデルは推論速度の制約から、高速かつ精密な動作制御が求められるタスクへの適用が困難である[2, 4]。これらの課題に対し、VLMによる高レベルの意味理解と、小規模ポリシーによる精密な動作生成を分離する階層的アプローチが提案されている[3, 5, 11]。

HAMSTERは、VLM（VILA-1.5-13B）を用いて2次元のエンドエフェクタ経路を生成し、これを低レベルポリシー（RVT-2や3D Diffuser Actor）へのガイダンスとして与える階層的VLAモデルである[3]。この設計により、VLMはセマンティックな軌道計画に専念し、低レベルポリシーは3次元空間での精密な制御に集中できる。HAMSTERは、シミュレーションデータや異なるエンボディメントのデータなど、アクションラベルを含まないオフドメインデータを活用できる点でも優れている。
...
本研究では、HAMSTERの階層的アーキテクチャにおける低レベルポリシーとしてManiFlowを採用し、VLMが生成した2次元パスをManiFlowの入力に統合する手法を提案する。従来のManiFlowは単一タスクに特化した学習を行い、学習環境と同一の条件で評価されてきた。本研究では、VILAによるパスガイダンスを導入することで、（1）同一環境における単一タスク精度の向上、および（2）学習環境と異なる条件への汎化性能の向上を検証する。
...
評価実験は、RoboTwin 2.0シミュレーション環境[8]上で実施する。学習環境（clean table条件またはcluttered table条件）と評価環境（cluttered table条件）の組み合わせにより6つの実験条件を設定し、パスガイダンスが単一タスク精度および汎化性能に与える効果を定量的に分析する。
...
現在フレームに対してVILAが生成したパスのみを使用する。具体的には、現在の観測画像にパスをオーバーレイした画像（current overlay）と、ロボット状態を入力としてManiFlowに与える。
...
VILAへの入力は、RGB画像と自然言語によるタスク指示である。タスク指示は、例えば「Pick up the hammer and beat the block」や「click the bell's top center on the table」のような形式で与えられる。
...
- **マルチモーダル統合**: 視覚入力、言語入力、固有受容感覚入力を効率的に統合する
...
パスオーバーレイ画像は、元のRGB画像にVILAが生成した2次元経路を視覚的に描画したものである。この設計により、ManiFlowのアーキテクチャ自体を変更することなく、高レベルの軌道ガイダンスを入力として追加できる。ManiFlowは、オーバーレイされた経路情報を視覚特徴として学習し、タスク実行に活用することが期待される。
...
タスク実行中、ロボットのエンドエフェクタや把持した物体が視野を遮ることで、オクルージョンが発生する場合がある。このような状況では、VILAが生成するパスの品質が低下し、不正確または不完全な経路が出力される可能性がある。

エピソード開始時（frame 0）では、通常オクルージョンが発生しておらず、タスク対象物体が明確に視認できる状態にある。したがって、この時点で生成されるパスは、タスク全体の軌道計画として信頼性が高いと考えられる。Memory Functionは、この初期パスを保持し続けることで、タスク実行中に現在パスの品質が低下した場合でも、タスク全体の文脈情報を参照可能にする。
...
Memory Functionにより、以下の効果が期待される：

1. **オクルージョン耐性の向上**: 現在フレームでパス生成が不安定になった場合でも、初期パスがタスク全体の軌道情報を提供する

2. **タスク文脈の保持**: 初期パスは、タスク開始時点での完全な軌道計画を含んでおり、タスクの全体像を把握する手がかりとなる

3. **長期的な計画情報の活用**: ManiFlowは16ステップ分のアクションを予測するが、初期パスを参照することで、より長期的な計画に基づいた動作生成が可能になる

#### Current pathのみとの比較

Current pathのみを使用する場合（パターン1）と、Memory Functionを使用する場合（パターン2）の違いを表2に示す。

**表2: パス入力パターンの比較**

| 項目 | Current pathのみ | Memory Function |
|------|-----------------|-----------------|
| 入力画像数 | 2枚（過去2ステップ） | 3枚（initial + 過去2ステップ） |
| パス更新頻度 | 16ステップごと | current: 16ステップごと / initial: 固定 |
| オクルージョン耐性 | 低い | 高い（初期パスを参照可能） |
| タスク全体の文脈 | 現在の局所情報のみ | 初期状態からの全体計画を保持 |
| 計算コスト | 低い | やや高い（入力画像が1枚増加） |

実験では、これら2つのパターンを比較し、Memory Functionの効果を定量的に検証する。
...
評価タスクとして、RoboTwin 2.0のシングルアームタスク（35タスク）の中から、難易度と動作パターンの多様性を考慮して3タスクを選択した。click_bellは単純な接触動作、move_can_potは標準的なPick-and-Place、beat_block_hammerは道具使用を要する2段階動作である。各タスクについて、RoboTwin 2.0のスクリプト化されたエキスパートポリシーを用いて50本の成功軌道を収集し、学習データとした。

本研究では、3つのモデル構成（オリジナルManiFlow、VILA + ManiFlow (current path)、VILA + ManiFlow (initial + current path)）と2つの学習環境（cluttered、clean）の組み合わせにより、6つの実験条件（C1〜C6）を設定した。全ての条件において、評価はcluttered table条件で実施した。条件C1〜C3はcluttered環境で学習・評価を行うin-domain設定であり、条件C4〜C6はclean環境で学習しcluttered環境で評価を行うcross-domain設定である。この設計により、以下の4つの仮説を検証する：**H1**（VILAパスガイダンスはin-domain精度を向上させる）、**H2**（VILAパスガイダンスはcross-domain汎化を向上させる）、**H3**（Memory FunctionはCurrent pathのみより優れる）、**H4**（パスガイダンスの効果はcross-domain条件でより顕著である）。
...
#### 4.2.1 Effect of Training Environment

【図挿入: 条件別成功率のグラフ - `/gscratch/scrubbed/naoto03/projects/HAMSTER-ManiFlow-Integration/analysis/outputs/figures/main_results_grouped.png`を使用】

Table 1に、3タスク×6条件における成功率を示す。この結果から、**H1**（in-domainでのパスガイダンス効果）と**H4**（clean環境学習の汎化能力）を検証する。
...
最も顕著な発見は、clean環境で学習した条件（C4-C6、平均13.0〜28.0%）がcluttered環境で学習した条件（C1-C3、平均9.3〜10.3%）より一貫して高い成功率を示したことである。この結果は当初の予想と逆であり、**H4を強く支持する**。特にBeat Block with Hammerタスクでは、C4（31.0%）およびC5（37.0%）がC1-C3（1.0〜6.0%）を大幅に上回った。
...
【図挿入: 学習曲線の比較 - `/gscratch/scrubbed/naoto03/projects/HAMSTER-ManiFlow-Integration/analysis/outputs/figures/training_curves_by_task.png`を使用】

この解釈を裏付けるため、学習曲線の分析を行った。Figure Xに示すように、全18条件で学習損失は十分に収束し（損失削減率99.9%）、学習自体は正常に完了している。しかし、clean環境（C4-C6）の初期損失がcluttered環境（C1-C3）より低い傾向が観察された。特にBeat Block with Hammerでは、clean環境の初期損失（2.25-2.28）がcluttered環境（2.51-2.53）より約10%低かった。収束後の最終損失は全条件で同等（0.002-0.004）であるにもかかわらず評価性能に大きな差が生じたことは、**学習損失の低さが必ずしも汎化性能の高さを意味しない**ことを示唆している。

#### 4.2.2 Effect of Input Representation Mode

【図挿入: 仮説検証結果 - `/gscratch/scrubbed/naoto03/projects/HAMSTER-ManiFlow-Integration/analysis/outputs/figures/hypothesis_h1_h2.png`を使用】

**H1**および**H2**を検証するため、ベースライン（C1, C4）とパスガイダンス条件（C2, C3, C5, C6）を比較した。

**In-domain条件（H1検証）**: C1からC2への変化は平均-0.3%、C1からC3への変化は平均+0.7%であり、VILAパスガイダンスによる一貫した改善は見られなかった。したがって、**H1は支持されなかった**。

**Cross-domain条件（H2検証）**: C4からC5への変化は平均+7.7%の改善を示し、3タスク中2タスク（Click Bell: +17.0%、Beat Block with Hammer: +6.0%）で改善が確認された。一方、C4からC6（initial+current）への変化は平均-7.3%と逆に性能が低下した。したがって、**H2はcurrent pathモードにおいて部分的に支持された**。

**H3**を検証するため、current pathのみ（C2, C5）とinitial+current（C3, C6）を比較した。In-domain条件（C2→C3）では平均+1.0%のわずかな改善が見られたが、cross-domain条件（C5→C6）では平均-15.0%という大幅な性能低下が観察された。この結果は、**H3が支持されなかった**ことを示す。

【図挿入: Memory Function効果の比較 - `/gscratch/scrubbed/naoto03/projects/HAMSTER-ManiFlow-Integration/analysis/outputs/figures/hypothesis_h3.png`を使用】

initial+current方式の性能低下について、以下の要因が考えられる。第一に、2つのオーバーレイ画像（initial + current）を入力することでモデルが処理すべき情報量が増加し、学習が困難になった可能性がある。第二に、エピソードが進むにつれてinitial path（frame 0時点）の情報が現在の状況と乖離し、誤った誘導を与えた可能性がある。特にcross-domain条件では、clean環境で生成されたinitial pathがcluttered評価環境と不整合を起こしやすいと推測される。

#### 4.2.3 Task-Specific Analysis

タスク間で結果のパターンが異なることが観察された。Click Bellでは、C5で大幅な改善（3.0%→20.0%）が見られた一方、他の条件では1.0〜4.0%と低迷した。Move Can to Potでは、条件間の差が比較的小さく（20.0〜27.0%）、パスガイダンスの効果が限定的であった。Beat Block with Hammerでは、C4（31.0%）およびC5（37.0%）で高い成功率を示したが、C3（1.0%）およびC6（11.0%）では低迷した。

これらの結果は、VILAパスガイダンスの効果がタスクの特性に依存することを示唆している。明確な軌道計画を必要とするタスク（Click Bell、Beat Block with Hammer）では、適切に生成されたパスが動作の精度向上に寄与する。一方、Move Can to Potのような標準的なPick-and-Placeタスクでは、動作パターンが比較的単純であるため、パスガイダンスによる追加的な改善が限定的であったと考えられる。

#### 4.2.4 Training Dynamics

【図挿入: 学習曲線詳細 - `/gscratch/scrubbed/naoto03/projects/HAMSTER-ManiFlow-Integration/analysis/outputs/figures/validation_loss_comparison.png`を使用、または作成が必要な場合は「タスク別・条件別の学習曲線を示す3パネルの図（横軸：エポック、縦軸：訓練損失）」と記載】

全18条件（3タスク×6条件）について501エポックの学習を実施し、学習曲線を分析した。Table 2に学習統計を示す。
