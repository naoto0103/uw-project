...
Evaluation experiments are conducted in the RoboTwin 2.0 simulation environment [8]. We establish six experimental conditions by combining training environments (clean table or cluttered table) with evaluation environments (cluttered table), and quantitatively analyze the effects of path guidance on single-task accuracy and generalization performance.
...
- **Multi-modal integration**: Efficiently integrates visual input, language input, and proprioceptive input
...
1. **Improved occlusion robustness**: Even when path generation becomes unstable in the current frame, the initial path provides trajectory information for the overall task

2. **Task context preservation**: The initial path contains the complete trajectory plan at task onset, serving as a cue for grasping the overall task picture

3. **Utilization of long-term planning information**: While ManiFlow predicts 16 steps of actions, reference to the initial path may enable motion generation based on longer-term planning

#### Comparison with Current Path Only

Table 2 shows the differences between using only the current path (Pattern 1) and using the Memory Function (Pattern 2).

**Table 2: Comparison of Path Input Patterns**
...
【Figure: Success rate by condition - Use `/gscratch/scrubbed/naoto03/projects/HAMSTER-ManiFlow-Integration/analysis/outputs/figures/main_results_grouped.png`】

Table 1 shows success rates across 3 tasks × 6 conditions. From these results, we verify **H1** (in-domain path guidance effect) and **H4** (generalization capability of clean environment training).
...
【Figure: Training curve comparison - Use `/gscratch/scrubbed/naoto03/projects/HAMSTER-ManiFlow-Integration/analysis/outputs/figures/training_curves_by_task.png`】

To support this interpretation, we analyzed training curves. As shown in Figure X, training loss converged sufficiently across all 18 conditions (99.9% loss reduction), indicating that training completed normally. However, we observed that initial loss for clean environments (C4-C6) tended to be lower than for cluttered environments (C1-C3). Particularly for Beat Block with Hammer, clean environment initial loss (2.25-2.28) was approximately 10% lower than cluttered environments (2.51-2.53). The fact that large evaluation performance differences emerged despite final losses being comparable across all conditions (0.002-0.004) suggests that **lower training loss does not necessarily indicate higher generalization performance**.

#### 4.2.2 Effect of Input Representation Mode

【Figure: Hypothesis verification results - Use `/gscratch/scrubbed/naoto03/projects/HAMSTER-ManiFlow-Integration/analysis/outputs/figures/hypothesis_h1_h2.png`】

To verify **H1** and **H2**, we compared baselines (C1, C4) with path guidance conditions (C2, C3, C5, C6).

**In-domain conditions (H1 verification)**: The change from C1 to C2 was -0.3% on average, and from C1 to C3 was +0.7% on average, showing no consistent improvement from VILA path guidance. Therefore, **H1 was not supported**.

**Cross-domain conditions (H2 verification)**: The change from C4 to C5 showed an average improvement of +7.7%, with improvements confirmed in 2 of 3 tasks (Click Bell: +17.0%, Beat Block with Hammer: +6.0%). In contrast, the change from C4 to C6 (initial+current) showed a -7.3% decrease in performance. Therefore, **H2 was partially supported for the current path mode**.

To verify **H3**, we compared current path only (C2, C5) with initial+current (C3, C6). In in-domain conditions (C2→C3), there was a slight improvement of +1.0% on average, but in cross-domain conditions (C5→C6), a substantial performance decrease of -15.0% on average was observed. This result indicates that **H3 was not supported**.

【Figure: Memory Function effect comparison - Use `/gscratch/scrubbed/naoto03/projects/HAMSTER-ManiFlow-Integration/analysis/outputs/figures/hypothesis_h3.png`】
...
【Figure: Training curve details - Use `/gscratch/scrubbed/naoto03/projects/HAMSTER-ManiFlow-Integration/analysis/outputs/figures/validation_loss_comparison.png`】

We conducted 501 epochs of training for all 18 conditions (3 tasks × 6 conditions) and analyzed training curves. Table 2 shows training statistics.
