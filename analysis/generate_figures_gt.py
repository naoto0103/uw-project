#!/usr/bin/env python3
"""
Generate publication-ready figures for GT Path trained models evaluation.

Outputs:
- Success rate bar charts (GT-C2, GT-C3, GT-C5, GT-C6)

Usage:
    python generate_figures_gt.py
    python generate_figures_gt.py --no-show
    python generate_figures_gt.py --format pdf
"""
...
# Color schemes for GT conditions
IN_DOMAIN_COLORS = ['#ff7f0e', '#2ca02c']  # GT-C2, GT-C3
CROSS_DOMAIN_COLORS = ['#9467bd', '#8c564b']  # GT-C5, GT-C6
...
def plot_main_results(
    ...
    # Clearer legend labels for GT conditions
    CONDITION_LABELS = {
    ...
    # Plot bars for each GT condition
    for i, cond_id in enumerate(gt_cond_ids):
        ...
        # Add value labels on top of bars (including 0)
    ...
    ax.set_title('GT Path Trained Models: Success Rates Across All Conditions')
    ax.set_xticks(x)
...
def plot_main_results_grouped(
    ...
    Layout: Two groups per task (in-domain: GT-C2, GT-C3, cross-domain: GT-C5, GT-C6)
    """
    success_table = get_success_rates_table(all_results)
    ...
    ax2.set_xlabel('Task')
    ax2.set_title('GT Path Cross-Domain (Trained on Clean, Eval on Cluttered)')
    ax2.set_xticks(x)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate publication-ready figures for GT Path trained models evaluation."
    )
    parser.add_argument(
        "--no-show",
        action="store_true",
    ...
    print("Loading GT Path trained models evaluation results...")
    all_results = load_all_results()

    ensure_output_dirs()

    print(f"\nGenerating GT Path figures (format: {fig_format})...")

    # Main results - all GT conditions
    plot_main_results(
        all_results,
        save_path=FIGURES_DIR / f"gt_main_results_all.{fig_format}",
        show=show,
        fig_format=fig_format,
    )

    # Main results - grouped by in-domain/cross-domain
    plot_main_results_grouped(
        all_results,
        save_path=FIGURES_DIR / f"gt_main_results_grouped.{fig_format}",
        show=show,
        fig_format=fig_format,
    )
