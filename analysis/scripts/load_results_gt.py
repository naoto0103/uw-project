"""
Data loading utilities for GT Path trained models evaluation results.
"""
...
if __name__ == "__main__":
    ...
    for task, task_results in all_results.items():
        ...
        for cond_id, cond in sorted(task_results.conditions.items()):
            print(f"  GT-Condition {cond_id}: {cond.success_rate_percent:.1f}% "
                  f"({cond.num_successes}/{cond.num_episodes})")

    print("\n" + "=" * 60)
    print("Success rates table (GT Path):")
    print("=" * 60)

    table = get_success_rates_table(all_results)

    # Print header (GT conditions: 2, 3, 5, 6)
    gt_cond_ids = [2, 3, 5, 6]
    header = "Task".ljust(20) + "".join(f"GT-C{i}".rjust(10) for i in gt_cond_ids)
    print(header)
    print("-" * len(header))
