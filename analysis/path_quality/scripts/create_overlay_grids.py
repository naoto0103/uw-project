#!/usr/bin/env python3
"""
Create grid visualization of overlay images for path quality analysis.

For each episode, creates a grid image showing overlay images at 16-frame intervals.
Frames where path generation failed (no corresponding .pkl file) are marked with a red X.

Usage:
    python create_overlay_grids.py
    python create_overlay_grids.py --tasks click_bell --conditions clean
    python create_overlay_grids.py --episodes 10  # Process first 10 episodes only
"""
...
OUTPUT_DIR = Path("/gscratch/scrubbed/naoto03/projects/HAMSTER-ManiFlow-Integration/analysis/path_quality/grids")
TARGET_TASKS = ["click_bell", "move_can_pot", "beat_block_hammer"]
CONDITIONS = ["evaluation_tasks_clean", "evaluation_tasks_cluttered"]
FRAME_INTERVAL = 16
COLS = 4
...
def create_episode_grid(condition, task, episode_num, frame_interval=FRAME_INTERVAL, cols=COLS):
    ...
    for frame_num in sample_frames:
        overlay_file = overlay_dir / f"overlay_{frame_num:04d}.png"
...
def main():
    parser = argparse.ArgumentParser(description="Create overlay grid visualizations")
    ...
    print("=" * 60)
    print("Creating path quality visualization grids")
    print("=" * 60)
    ...
    for cond_short in args.conditions:
        condition = f"evaluation_tasks_{cond_short}"
        print(f"\n### {cond_short} ###")
