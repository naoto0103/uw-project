#!/usr/bin/env python3
"""
Compare VILA vs Qwen3-VL for new task: "Pick up the apple and put it behind the hammer"
"""
import cv2
import numpy as np
import pickle
...
qwen3_path_file = '/home/naoto/HAMSTER-ManiFlow-Integration/HAMSTER/results/qwen3_new_task_path.pkl'

if not os.path.exists(vila_path_file):
    print(f"\nERROR: VILA path file not found: {vila_path_file}")
    print("Please run test_new_task_vila.py first")
    exit(1)

if not os.path.exists(qwen3_path_file):
    print(f"\nERROR: Qwen3 path file not found: {qwen3_path_file}")
    print("Please run test_new_task_qwen3.py first")
    exit(1)

vila_data = pickle.load(open(vila_path_file, 'rb'))
vila_path = vila_data['episode_0']
vila_instruction = vila_data['instruction']

qwen3_data = pickle.load(open(qwen3_path_file, 'rb'))
qwen3_path = qwen3_data['episode_0']
qwen3_instruction = qwen3_data['instruction']

print(f"\nTask: {vila_instruction}")
print(f"VILA path: {len(vila_path)} waypoints")
print(f"Qwen3 path: {len(qwen3_path)} waypoints")
...
qwen3_vis = draw_path_on_frame(
    frame,
    qwen3_path,
    "Qwen3-VL-8B (Zero-shot JSON)",
    f"{len(qwen3_path)} waypoints"
)

# Create horizontal comparison
comparison_h = np.hstack([vila_vis, qwen3_vis])
...
# Add overall title
font = cv2.FONT_HERSHEY_SIMPLEX
overall_title = f'Task: "{vila_instruction}"'
font_scale = 0.5
font_thickness = 1
...
cv2.imwrite(f'{output_dir}/new_task_comparison.png',
            cv2.cvtColor(final_comparison, cv2.COLOR_RGB2BGR))

print("\nSaved visualization:")
print(f"  - {output_dir}/new_task_comparison.png")
...
print("\nQwen3-VL-8B (Zero-shot JSON optimized):")
for i, (x, y, g) in enumerate(qwen3_path):
    gripper_str = "CLOSE" if g == GRIPPER_CLOSE else "OPEN "
    print(f"  Waypoint {i}: ({x:.3f}, {y:.3f}) - {gripper_str}")

qwen3_length = sum(np.sqrt((qwen3_path[i+1][0]-qwen3_path[i][0])**2 +
                           (qwen3_path[i+1][1]-qwen3_path[i][1])**2)
                   for i in range(len(qwen3_path)-1))
