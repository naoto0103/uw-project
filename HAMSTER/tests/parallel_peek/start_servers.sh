#!/bin/bash
#
# Start 4 PEEK (VILA-3B) servers on different GPUs (ports 8010-8013)
#
# Usage:
#   ./start_servers.sh
#
...
#   - PEEK model downloaded to HuggingFace cache
#
...
PROJECT_DIR="$HAMSTER_DIR/.."
PEEK_DIR="$PROJECT_DIR/PEEK"
PEEK_VLM_DIR="$PEEK_DIR/peek_vlm"

# Configuration
NUM_GPUS=4
BASE_PORT=8010  # Different from VILA-13B (8000-8003)
MODEL_PATH="/gscratch/scrubbed/naoto03/.cache/huggingface/models--memmelma--peek_3b/snapshots/12df7a04221a50e88733cd2f1132eb01257aba0d"
...
echo "============================================================"
echo "Starting $NUM_GPUS PEEK (VILA-3B) Servers (Parallel Mode)"
echo "============================================================"
...
if [ ! -d "$MODEL_PATH" ]; then
    ...
    echo "  huggingface-cli download memmelma/peek_3b"
    exit 1
fi
...
# Check if PEEK VLM exists
if [ ! -d "$PEEK_VLM_DIR" ]; then
    echo "ERROR: PEEK VLM not found at $PEEK_VLM_DIR"
    exit 1
fi
...
for GPU_ID in $(seq 0 $((NUM_GPUS - 1))); do
    ...
    echo "Starting PEEK server on GPU $GPU_ID, port $PORT..."
    echo "  Log: $LOG_FILE"

    # Start server in background using PEEK's vila_server.py
    # PYTHONPATH includes both VILA site-packages and PEEK VLM
    singularity exec --nv \
        --bind /gscratch/:/gscratch/:rw \
        --env PYTHONPATH="$SITE_PACKAGES_VILA:$PEEK_VLM_DIR:$HAMSTER_DIR/VILA" \
        --env PYTHONNOUSERSITE=1 \
        --env HF_HOME="$HF_HOME" \
        --env TRANSFORMERS_CACHE="$HF_HOME" \
        --env CUDA_VISIBLE_DEVICES="$GPU_ID" \
        "$SIF_PATH" \
        python -W ignore "$PEEK_VLM_DIR/scripts/vila_server.py" \
            --host "0.0.0.0" \
            --port "$PORT" \
