...
def worker_process(
    ...
    while not shutdown_event.is_set():
        try:
            ...
            if not raw_exists and task.episode_num >= 37:
                print(f"[Worker {worker_id}] DEBUG: {task.task_name}/ep{task.episode_num}/frame{task.frame_num} raw_exists={raw_exists} path={task.raw_output_path}")
