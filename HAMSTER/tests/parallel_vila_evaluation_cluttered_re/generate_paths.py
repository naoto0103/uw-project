...
# Log directory for real-time logging
LOG_DIR = Path(__file__).parent / "logs"


def write_log(log_file: Path, message: str):
    """Write a log message with timestamp, flushing immediately."""
...
def parse_vila_path(response_text: str) -> Optional[List[Tuple[float, float, int]]]:
    ...
    if ans_match:
        ...
        # Check if it looks like coordinate data (starts with a number or parenthesis)
...
def worker_process(
    ...
    Args:
        ...
        log_file: Path to log file for real-time logging
    """
    ...
    while not shutdown_event.is_set():
        try:
            ...
            try:
                ...
                # Log the result (only for actual generation attempts, not cached)
                status = "SUCCESS" if path is not None else "FAIL"
                num_waypoints = len(path) if path else 0
                log_msg = f"[W{worker_id}] {status} | {task.task_name}/ep{task.episode_num:02d}/frame{task.frame_num:04d} | {num_waypoints} WP | {processing_time:.1f}s"
                ...
                # Log error
                log_msg = f"[W{worker_id}] ERROR | {task.task_name}/ep{task.episode_num:02d}/frame{task.frame_num:04d} | {str(e)[:50]}"
                write_log(log_file, log_msg)
...
def main():
    ...
    # Count already processed (based on raw_output existence, matching worker skip logic)
    already_processed = sum(
        1 for f in all_frames
        if os.path.exists(f.raw_output_path)
    )
    ...
    log_file = LOG_DIR / f"generation_{log_timestamp}.log"
    ...
    for i in range(args.num_gpus):
        p = mp.Process(
            ...
            args=(i, task_queue, result_queue, shutdown_event, log_file)
        )
        p.start()
        workers.append(p)
