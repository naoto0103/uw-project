...
def parse_vila_path(response_text: str) -> Optional[List[Tuple[float, float, int]]]:
    ...
    if ans_match:
        ...
        # Fallback: No <ans> tag, try to parse the raw response
        # Handle pattern like: "0.39, 0.12, <action>Close Gripper</action>, (0.45, 0.18), ..."
        ans_content = response_text.strip()

        # Remove orphan </ans> tag (when <ans> is missing but </ans> exists)
        ans_content = re.sub(r'\]?\s*</ans>\s*$', ']', ans_content, flags=re.IGNORECASE)

        # Check if it looks like coordinate data (starts with a number)
        if not re.match(r'^\(?\d', ans_content):
            return None

        # Fix missing parentheses at the start: "0.39, 0.12," -> "(0.39, 0.12),"
        # Handle cases: "0.43, 0.60, <action>", "0.43, 0.60 <action>", "0.43, 0.60, (X, X)"
        fix_match = re.match(r'^(\d+\.?\d*)\s*,\s*(\d+\.?\d*)\s*,?\s*(?=<action>|\()', ans_content)
        if fix_match:
            x, y = fix_match.group(1), fix_match.group(2)
            remainder = ans_content[fix_match.end():]
            ans_content = f'({x}, {y}), ' + remainder

        # Fix missing opening paren only: "0.29, 0.44), ..." -> "(0.29, 0.44), ..."
        fix_open_paren = re.match(r'^(\d+\.?\d*)\s*,\s*(\d+\.?\d*)\)', ans_content)
        if fix_open_paren:
            x, y = fix_open_paren.group(1), fix_open_paren.group(2)
            ans_content = f'({x}, {y})' + ans_content[fix_open_paren.end():]

        # Wrap in list brackets if not already
        ans_content = ans_content.strip()
        if not ans_content.startswith('['):
            ans_content = '[' + ans_content
        if not ans_content.endswith(']'):
            # Remove trailing comma if present
            ans_content = ans_content.rstrip(',') + ']'
    ...
    for point in keypoints:
        # Handle non-tuple elements (e.g., bare floats)
...
def worker_process(
    ...
    # Setup log file for this worker
    log_file = os.path.join(log_dir, f"worker_{worker_id}_generation.log")

    def log_generation(task: FrameTask, success: bool):
        """Write generation result to log file immediately (only for actual generation attempts)."""
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        status = "SUCCESS" if success else "FAILED"
        log_line = f"[{timestamp}] [{status}] {task.task_name}/episode_{task.episode_num:02d}/frame_{task.frame_num:04d}\n"
    ...
    while not shutdown_event.is_set():
        try:
            ...
            # Check file existence for skip logic
            raw_exists = os.path.exists(task.raw_output_path)
            path_exists = os.path.exists(task.output_path)

            # Skip if raw_output already exists (already processed, regardless of path success)
            if raw_exists:
            ...
            try:
                ...
                log_generation(task, success=path is not None)

            except Exception as e:
                processing_time = time.time() - start_time

                # Save raw_response if available (to prevent re-processing)
                if 'raw_response' in dir() and raw_response:
                    try:
                        os.makedirs(os.path.dirname(task.raw_output_path), exist_ok=True)
                        with open(task.raw_output_path, 'w') as f:
                            f.write(raw_response)
                ...
                log_generation(task, success=False)
...
def main():
    ...
    # Count already processed (path exists = already done)
    already_processed = sum(
        1 for f in all_frames
        if os.path.exists(f.output_path)
    )
    # Count frames needing re-generation (raw exists but path doesn't)
    needs_regeneration = sum(
        1 for f in all_frames
        if os.path.exists(f.raw_output_path) and not os.path.exists(f.output_path)
    )
    print(f"Already processed (path exists): {already_processed}")
    print(f"Needs re-generation (raw exists, path missing): {needs_regeneration}")
    ...
    for i in range(args.num_gpus):
        p = mp.Process(
            ...
            args=(i, task_queue, result_queue, shutdown_event, str(log_dir))
        )
        p.start()
        workers.append(p)
