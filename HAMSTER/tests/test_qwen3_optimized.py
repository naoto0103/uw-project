#!/usr/bin/env python3
"""
Qwen3-VL optimized path generation using recommended prompt format
Based on Qwen3-VL 2d_grounding cookbook
"""
import base64
import cv2
import numpy as np
...
def test_qwen3_optimized():
    """Test Qwen3-VL with optimized prompt based on cookbook recommendations"""
    ...
    # Task instruction
    task_description = "Pick up the apple from the messy table"
...
Think step by step and output the complete trajectory in JSON format."""
    ...
    print("\n" + "="*60)
    print("QWEN3-VL RESPONSE (Optimized Prompt):")
    print("="*60)
    print(response_text)
    print("="*60)
    ...
    if hamster_path:
        ...
        for i, wp in enumerate(hamster_path[:3]):
            gripper_str = "CLOSE" if wp[2] == GRIPPER_CLOSE else "OPEN"
            print(f"    {i}: ({wp[0]:.3f}, {wp[1]:.3f}) - {gripper_str}")

        if len(hamster_path) > 3:
            print(f"  ...")
            print(f"  Last waypoint:")
            wp = hamster_path[-1]
            gripper_str = "CLOSE" if wp[2] == GRIPPER_CLOSE else "OPEN"
            print(f"    {len(hamster_path)-1}: ({wp[0]:.3f}, {wp[1]:.3f}) - {gripper_str}")
        ...
        # Save path
        output_path = "/home/naoto/HAMSTER-ManiFlow-Integration/HAMSTER/results/qwen3_optimized_path.pkl"
