...
Uses HAMSTER original drawing style (same as training data):
- jet colormap for path lines (blue -> cyan -> green -> yellow -> red)
- Red markers for gripper close (at state change points only)
- Blue markers for gripper open (at state change points only)
- Path interpolation with 100 subdivisions
...
# Add training_data directory to path for overlay_utils import
sys.path.insert(0, str(Path(__file__).parent / "training_data"))
...
def visualize_and_create_video(
    ...
    Uses HAMSTER original drawing style (same as training data generation).
    ...
    output_frames_dir = episode_dir / "frames_with_paths_hamster_style"
    output_video = episode_dir / f"{task_name}_ep{episode_num:02d}_path_video_hamster_style.mp4"
    ...
    for i, frame_file in enumerate(frame_files):
        ...
        # Draw path on frame using HAMSTER original style
        frame_with_path = draw_path_on_image_hamster_style(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create visualization videos for RoboTwin 2.0 single-arm VILA paths (HAMSTER style)"
    )
    parser.add_argument("--tasks", type=str, nargs="+", default=SINGLE_ARM_TASKS,
                        help="Tasks to process")
    parser.add_argument("--episodes", type=int, default=1,
                        help="Number of episodes per task (or end episode if --start-episode is set)")
    ...
    parser.add_argument("--num-subdivisions", type=int, default=100,
    ...
    print("=" * 80)
    print("RoboTwin 2.0 Single-Arm Video Creation (VILA - HAMSTER Style)")
    print("=" * 80)
    ...
    print()
    print("Drawing specification (HAMSTER original - same as training data):")
    ...
    for task_idx, task_name in enumerate(args.tasks):
        ...
        for ep_idx in range(args.start_episode, args.episodes):
    ...
    print()
    print("=" * 80)
    print("Video Creation Summary (HAMSTER Style)")
    print("=" * 80)
    ...
    summary_file = base_dir / "video_creation_summary_vila_hamster_style.json"
    with open(summary_file, 'w') as f:
        json.dump({
            "model": "VILA-1.5-13B (HAMSTER finetuned)",
            "arm_type": "single",
            "drawing_style": "HAMSTER original (jet colormap, same as training data)",
